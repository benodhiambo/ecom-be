...
class ProductController
{
    ...
    public function getFilteredProducts(): ResponseInterface
    {
        try {
            
            $post_data = $this->controller->getPostData();

            $category_id = $post_data['categories'][0]['category_id'];
            $filters = $post_data['categories'][0]['filters'];

            $results = $this->model->getFilteredProducts($category_id, $filters);
            ...
            $data = ['error' => 'Get Filtered Products Error - ' . $e->getMessage()];
            return $this->controller->setErrorResponse($data);
        }
    }
